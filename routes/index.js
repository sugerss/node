...
router.post('/home',function(req,res){
	...
	var banner_text=req.body['banner'];
	var services_text=req.body['services'];
	var china_text=req.body['china'];
	pool.query(`insert into home(banner_text,services_text,china_text) values('${banner_text}','${services_text}','${china_text}')`,function(err,rows){
		if (err) throw err;
		if(rows){
			res.send('上传成功')
		}			
...
router.get('/datas',function(req,res){
	res.header("Access-Control-Allow-Origin", "*");
	pool.query('select banner_text,services_text,china_text from home',function(err,rows){
		if(err) throw err;
		res.send(rows);
	})
});
